...
func renderHelp() string {
	...
	sb.WriteString("  " + cmd.Render("/stream") + dim.Render("              Toggle live streaming") + "\n")
	sb.WriteString("  " + cmd.Render("/think") + dim.Render(" [level]       Set thinking level") + "\n")
	sb.WriteString("  " + cmd.Render("/fast") + dim.Render(" [on|off]       Toggle fast mode") + "\n")
	sb.WriteString("  " + cmd.Render("/verbose") + dim.Render(" [mode]     Set verbose mode") + "\n")
	sb.WriteString("  " + cmd.Render("/processes") + dim.Render("           Background processes") + "\n")
	sb.WriteString("  " + cmd.Render("/cron") + dim.Render("                Scheduled jobs") + "\n")
...
func renderToolsList() string {
	...
	sb.WriteString("  " + dim.Render("Web") + "       " + tool.Render("web_search") + dim.Render(", ") + tool.Render("web_fetch") + "\n")
	sb.WriteString("  " + dim.Render("Process") + "   " + tool.Render("process_start") + dim.Render(", ") + tool.Render("process_list") + dim.Render(", ") + tool.Render("process_poll") + dim.Render(", ") + tool.Render("process_kill") + "\n")
	sb.WriteString("  " + dim.Render("Docs") + "      " + tool.Render("pdf_read") + "\n")
	sb.WriteString("  " + dim.Render("Schedule") + "  " + tool.Render("cron_add") + dim.Render(", ") + tool.Render("cron_list") + dim.Render(", ") + tool.Render("cron_remove") + "\n")
	sb.WriteString("  " + dim.Render("Advanced") + "  " + tool.Render("llm_task") + dim.Render(", ") + tool.Render("apply_patch") + "\n")
	sb.WriteString("\n")
	sb.WriteString("  " + dim.Render("+ integration tools (GitHub, Slack, Docker, etc.)") + "\n")

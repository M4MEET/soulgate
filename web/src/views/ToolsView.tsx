...
const FEATURED_HUB_ITEMS: HubItem[] = [
  // Skills
  { name: 'kubernetes-ops',  type: 'skill', description: 'Kubernetes cluster management instructions',          category: 'infra',      author: 'community',  downloads: 510,  rating: 4.5, tags: ['k8s', 'devops'] },
  { name: 'code-review',     type: 'skill', description: 'AI-powered code review with security and quality checks',  category: 'dev',  author: 'soulgate',   downloads: 980,  rating: 4.7, tags: ['code', 'review'] },
  { name: 'debugging',       type: 'skill', description: 'Systematic debugging workflow for complex issues',    category: 'dev',        author: 'soulgate',   downloads: 720,  rating: 4.6, tags: ['debug', 'workflow'] },
  { name: 'git-ops',         type: 'skill', description: 'Git operations: branch, commit, PR automation',        category: 'dev',        author: 'soulgate',   downloads: 750,  rating: 4.6, tags: ['git', 'automation'] },
  // Tools
  { name: 'web-scraper',     type: 'tool', kind: 'plugin',    description: 'Scrape and extract structured data from websites',    category: 'network',    author: 'soulgate',   downloads: 1200, rating: 4.8, tags: ['web', 'scraping'] },
  { name: 'pdf-ocr',         type: 'tool', kind: 'plugin',    description: 'Extract text from scanned PDFs using OCR',            category: 'filesystem', author: 'community',  downloads: 850,  rating: 4.5, tags: ['pdf', 'ocr'] },
  { name: 'db-query',        type: 'tool', kind: 'plugin',    description: 'Query SQLite, PostgreSQL, and MySQL databases',        category: 'data',       author: 'community',  downloads: 620,  rating: 4.4, tags: ['database', 'sql'] },
  { name: 'image-gen',       type: 'tool', kind: 'plugin',    description: 'Generate images using DALL-E, Stable Diffusion, Flux', category: 'creative',  author: 'soulgate',   downloads: 1500, rating: 4.9, tags: ['image', 'ai'] },
  { name: 'github-mcp',      type: 'tool', kind: 'mcp',       description: 'GitHub MCP server for repo, issue, and PR access',    category: 'dev',        author: 'soulgate',   downloads: 890,  rating: 4.7, tags: ['github', 'mcp'] },
  { name: 'telegram',        type: 'tool', kind: 'connector',  description: 'Telegram bot connector for messaging integration',    category: 'network',    author: 'soulgate',   downloads: 640,  rating: 4.5, tags: ['telegram', 'bot'] },
  { name: 'email-send',      type: 'tool', kind: 'plugin',    description: 'Send emails via SMTP or SendGrid/Mailgun APIs',       category: 'network',    author: 'community',  downloads: 430,  rating: 4.3, tags: ['email', 'smtp'] },
  { name: 'speech-to-text',  type: 'tool', kind: 'plugin',    description: 'Transcribe audio files using Whisper or Deepgram',     category: 'ai',         author: 'soulgate',   downloads: 820,  rating: 4.7, tags: ['audio', 'stt'] },
  { name: 'notion-sync',     type: 'tool', kind: 'mcp',       description: 'Read and write Notion pages and databases',            category: 'productivity', author: 'community', downloads: 690,  rating: 4.6, tags: ['notion', 'sync'] },
  { name: 's3-storage',      type: 'tool', kind: 'plugin',    description: 'Upload, download, and manage files in S3/R2/MinIO',    category: 'infra',      author: 'community',  downloads: 410,  rating: 4.3, tags: ['s3', 'cloud'] },
  { name: 'calendar-sync',   type: 'tool', kind: 'connector',  description: 'Sync with Google Calendar, Outlook, and CalDAV',      category: 'productivity', author: 'community', downloads: 380,  rating: 4.2, tags: ['calendar', 'sync'] },
  // Agents
  { name: 'code-reviewer',   type: 'agent', description: 'Automated code review agent with configurable rules',   category: 'dev',    author: 'soulgate',   downloads: 560,  rating: 4.6, tags: ['code', 'review'] },
  { name: 'devops-bot',      type: 'agent', description: 'DevOps automation agent for CI/CD and infra tasks',     category: 'infra',  author: 'community',  downloads: 340,  rating: 4.4, tags: ['devops', 'automation'] },
  { name: 'research-agent',  type: 'agent', description: 'Research agent that gathers and synthesizes information', category: 'ai',    author: 'soulgate',   downloads: 470,  rating: 4.5, tags: ['research', 'ai'] },
];

type HubTab = 'skills' | 'tools' | 'agents';
...
function HubItemCard({ item, isInstalled, isInstalling, onInstall }: {
  item: HubItem;
  isInstalled: boolean;
  ...
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-xl bg-zinc-800/30 border border-zinc-700/30 hover:border-zinc-600/40 transition-all">
      <div className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700/50 flex items-center justify-center flex-shrink-0">
        {item.type === 'skill' ? <BookOpen size={14} className="text-emerald-400/60" /> :
         item.type === 'agent' ? <Bot size={14} className="text-indigo-400/60" /> :
         <Cpu size={14} className="text-fuchsia-400/60" />}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-zinc-200 font-mono">{item.name}</span>
          {item.kind && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-700/50 text-zinc-400">{item.kind}</span>
          )}
...
function HubPanel({ installed, onRefresh }: { installed: InstalledHubItem[]; onRefresh: () => void }) {
  ...
  const install = async (item: HubItem) => {
    const installName = item.type ? `${item.type}:${item.name}` : item.name;
    setInstalling(item.name);
    try {
      const res = await hubInstall(installName);
      if (res.error) toast.error(res.error);
      else { toast.success(`${item.name} installed`); onRefresh(); }
    } catch (err) { toast.error(`Install failed: ${err}`); }
    finally { setInstalling(null); }
  };

  const uninstall = async (item: InstalledHubItem) => {
    const uninstallName = item.type ? `${item.type}:${item.name}` : item.name;
    setUninstalling(item.name);
    try {
      const res = await hubUninstall(uninstallName, item.type);
      if (res.error) toast.error(res.error);
      else { toast.success(`${item.name} uninstalled`); onRefresh(); }
  ...
  // Filter by selected hub tab
  const filteredItems = displayItems.filter(item => {
    switch (hubTab) {
      case 'skills': return item.type === 'skill';
      case 'tools':  return item.type === 'tool' || (!item.type && !['skill', 'agent'].includes(item.type || ''));
      case 'agents': return item.type === 'agent';
      default: return true;
    }
  });
  ...
  // Count items per tab
  const countByTab = (tab: HubTab) => {
    const tabType = tab === 'skills' ? 'skill' : tab === 'tools' ? 'tool' : 'agent';
    return displayItems.filter(i => i.type === tabType || (!i.type && tab === 'tools')).length;
  };

  const tabConfig: { key: HubTab; label: string; icon: typeof BookOpen }[] = [
      ...
      <div className="rounded-xl border border-zinc-700/40 bg-zinc-900/30 overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-700/30 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-[11px] text-zinc-500">Browse skills, tools, and agents from the community</div>
          </div>
        ...
        {tabConfig.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setHubTab(key)}
            className={`flex-1 px-3 py-2 rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
              hubTab === key
                ? key === 'skills' ? 'bg-emerald-500/15 text-emerald-300' :
                  key === 'tools' ? 'bg-fuchsia-500/15 text-fuchsia-300' :
                  'bg-indigo-500/15 text-indigo-300'
                : 'text-zinc-500 hover:text-zinc-300'
            }`}>
            <Icon size={12} /> {label}
            <span className="text-[10px] opacity-60">({countByTab(key)})</span>
          </button>
        ))}
      </div>

      {/* Installed items for current tab */}
      {filteredInstalled.length > 0 && (
        <div className="rounded-xl border border-emerald-500/15 bg-emerald-500/[0.03] overflow-hidden">
          <div className="px-5 py-3 border-b border-emerald-500/10 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-emerald-400">Installed ({filteredInstalled.length})</span>
          </div>
            ...
            {filteredInstalled.map(item => (
              <div key={`${item.type}-${item.name}`} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-zinc-800/30 border border-zinc-700/20">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.kind && <span className="text-[10px] text-zinc-700 ml-2">({item.kind})</span>}
                  <span className="text-[10px] text-zinc-700 ml-2">{item.type}</span>
                </div>
                <button onClick={() => uninstall(item)} disabled={uninstalling === item.name}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">
            {showFeatured ? 'Featured' : `Results (${filteredItems.length})`}
          </span>
          {showFeatured && (
            <span className="text-[10px] text-zinc-600">
              {hubTab === 'skills' ? 'Behavioral instruction sets' :
               hubTab === 'tools' ? 'Plugins, MCP servers, connectors, and scripts' :
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-8 text-zinc-600 text-xs">
            No {hubTab} found. Try searching or browse another category.
          </div>
        ) : (

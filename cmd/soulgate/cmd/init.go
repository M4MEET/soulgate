...
For an interactive setup with custom configuration, use: soulgate onboarding
...
Tip: Use 'soulgate onboarding' for a guided configuration experience.`,
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  - %s/audit.jsonl (audit log)\n", workspace.ConfigDir)
	fmt.Printf("  - %s/ (plugins directory)\n", workspace.Config.Plugins.Dir)
	fmt.Println()
	fmt.Println("⚠️  Using default configuration. For guided setup, run:")
	...
	fmt.Println()
	fmt.Println("Or run the onboarding wizard with: soulgate onboarding")
